<?php
ini_set('date.timezone', 'Etc/GMT+7');
    ...
    public function getTimeStamp($query)
    {
        ...
        if ($query == 'now') {
            $workflows->result($query,
                ...
                "美国时间：" . date('Y年m月d日 H时i分s秒', time()),
                '',
                'icon.png', false);
            $workflows->result($query,
                date('Y年m月d日 H时i分s秒', strtotime("+15 hours")),
                "中国时间：" . date('Y年m月d日 H时i分s秒', strtotime("+15 hours")),
                '',
                'icon.png', false);

            $workflows->result($query,
                date('Y-m-d H:i:s', time()),
                "美国时间：" . date('Y-m-d H:i:s', time()),
                '',
                'icon.png', false);

            $workflows->result($query,
                date('YmdHis', time()),
                "美国时间：" . date('YmdHis', time()),
                '',
                'icon.png', false);
        ...
        if (is_numeric($query)) {
            ...
            if (preg_match('/^\d{1,10}$/', $query)) {
                $workflows->result($query,
                    ...
                    '美国时间：' . date('Y-m-d H:i:s', $query),
                    $this->getDifference($query, time()),
                    'icon.png', false);
		
		$workflows->result($query,
                    date('Y-m-d H:i:s', $query + strtotime("+15 hours") - time()),
                    '中国时间：' . date('Y-m-d H:i:s', $query + strtotime("+15 hours") - time()),
                    $this->getDifference($query, time()),
                    'icon.png', false);
                echo $workflows->toxml();
            }
            ...
            if (preg_match('/^\d{11,13}$/', $query)) {
                ...
                $workflows->result($resQuery,
                    ...
                    '美国时间：' . date('Y-m-d H:i:s', $resQuery),
                    $this->getDifference($resQuery, time()),
                    'icon.png', false);
                $workflows->result($resQuery,
                    date('Y-m-d H:i:s', $resQuery + strtotime("+15 hours") - time()),
                    '中国时间：' . date('Y-m-d H:i:s', $resQuery + strtotime("+15 hours") - time()),
                    $this->getDifference($resQuery, time()),
                    'icon.png', false);
                echo $workflows->toxml();
            }
        ...
        if ($this->isDateTime($query)) {
            $workflows->result($query,
                ...
                '中国时间：' . date('Y-m-d H:i:s', strtotime($query) + strtotime("+15 hours") - time()),
                '',
                'icon.png', false);
            $workflows->result($query,
                strtotime($query),
